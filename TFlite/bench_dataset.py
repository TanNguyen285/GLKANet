...
Đặc điểm:
    ...
    - Nghỉ đúng 3 giây giữa lúc benchmark xong model A và trước khi bắt đầu model B,
      để nhiệt độ CPU/GPU và bộ nhớ ổn định lại, tránh benchmark model sau bị ăn theo
      hiệu ứng cache/nhiệt của model trước.
    - Chạy batch=1 tuần tự cho cả 2 backend để so sánh công bằng theo per-image latency.
...
Chạy (trong venv-tflite vì cần cả onnxruntime lẫn tensorflow/tflite_runtime):
    ...
    python bench_dataset.py ^
        --dataset-yaml "C:\\Users\\ThisPC\\Documents\\GitHub\\Simple_GLKA\\glkanet\\configs\\dataset.yaml" ^
        --onnx "C:\\...\\best_deploy.onnx" ^
        --tflite "C:\\...\\best_deploy_float32.tflite" ^
        --input-size 224

    Có thể benchmark chỉ 1 model (bỏ --onnx hoặc --tflite tương ứng).
...
def benchmark_onnx_dataset(
    ...
    # ── Load trước toàn bộ ảnh thành tensor (đọc ảnh 1 lần, không tính vào thời gian đo) ──
...
def main():
    ...
    ap.add_argument("--tflite", default=None, help="Đường dẫn model .tflite (bỏ qua nếu không muốn benchmark TFLite)")
    ...
    if not args.onnx and not args.tflite:
        print("❌ Cần truyền ít nhất --onnx hoặc --tflite (có thể truyền cả 2 để so sánh).")
        sys.exit(1)
    ...
    if args.onnx and args.tflite:
        print(f"\n⏸  Nghỉ {PAUSE_BETWEEN_MODELS_S}s giữa 2 model để hệ thống ổn định lại...")
    ...
    if args.tflite:
        if not Path(args.tflite).exists():
            ...
            sys.exit(1)
        print(f"\n{'#'*70}\n# BENCHMARK TFLITE: {args.tflite}\n{'#'*70}")
        t_start = time.perf_counter()
        r = benchmark_tflite_dataset(args.tflite, image_paths, labels, args.input_size, mean, std)
        r["wall_clock_total_s"] = round(time.perf_counter() - t_start, 3)
        all_results["tflite"] = {"model_path": str(Path(args.tflite).resolve()), "result": r}
        print_result("TFLite", r)

    # ── So sánh nhanh nếu chạy cả 2 ──
    if "onnx" in all_results and "tflite" in all_results:
        ro, rt = all_results["onnx"]["result"], all_results["tflite"]["result"]
        if "error" not in ro and "error" not in rt:
            print(f"\n{'='*70}\n  SO SÁNH NHANH\n{'='*70}")
            print(f"  {'':<22} {'ONNX':>15} {'TFLite':>15}")
            print(f"  {'Latency mean (ms)':<22} {ro['latency_mean_ms']:>15} {rt['latency_mean_ms']:>15}")
            print(f"  {'Throughput (FPS)':<22} {ro['throughput_fps']:>15} {rt['throughput_fps']:>15}")
            print(f"  {'Accuracy (%)':<22} {ro['accuracy']*100:>14.2f}% {rt['accuracy']*100:>14.2f}%")
            faster = "ONNX" if ro['latency_mean_ms'] < rt['latency_mean_ms'] else "TFLite"
            diff_pct = abs(ro['latency_mean_ms'] - rt['latency_mean_ms']) / max(ro['latency_mean_ms'], rt['latency_mean_ms']) * 100
            print(f"  → {faster} nhanh hơn {diff_pct:.1f}%")
